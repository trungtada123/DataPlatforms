param(
    ...
    [string]$DumpRelativePath = "exports/ssi_market_stock_only.dump",
...
$dumpPath = (Resolve-Path (Join-Path $repoRoot $DumpRelativePath)).Path
$dumpFileName = Split-Path -Leaf $dumpPath
$envMap = Get-EnvMap -Path $envPath

if (-not (Test-Path $dumpPath)) {
    throw "Không tìm thấy dump market tại: $dumpPath"
}
...
try {
    ...
    docker exec -e "PGPASSWORD=$postgresPassword" $PostgresContainer `
        ...
        -U $postgresUser -d $postgresDb "/restore/$dumpFileName"
