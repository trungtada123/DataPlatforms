...
DDL_STATEMENTS: list[str] = [
    ...
    CREATE TABLE IF NOT EXISTS daily_stock_raw (
        ...
        anomaly_ref_zero BOOLEAN NOT NULL DEFAULT FALSE,
        anomaly_ceiling_zero BOOLEAN NOT NULL DEFAULT FALSE,
        anomaly_floor_zero BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    "ALTER TABLE daily_stock_raw ADD COLUMN IF NOT EXISTS anomaly_ref_zero BOOLEAN NOT NULL DEFAULT FALSE",
    "ALTER TABLE daily_stock_raw ADD COLUMN IF NOT EXISTS anomaly_ceiling_zero BOOLEAN NOT NULL DEFAULT FALSE",
    "ALTER TABLE daily_stock_raw ADD COLUMN IF NOT EXISTS anomaly_floor_zero BOOLEAN NOT NULL DEFAULT FALSE",
    "ALTER TABLE daily_stock_raw ADD COLUMN IF NOT EXISTS anomaly_reason VARCHAR(255)",
    "COMMENT ON COLUMN daily_stock_raw.anomaly_ref_zero IS 'Cờ dữ liệu SSI bất thường: RefPrice gốc bằng 0 (được chuẩn hóa thành NULL).' ",
    "COMMENT ON COLUMN daily_stock_raw.anomaly_ceiling_zero IS 'Cờ dữ liệu SSI bất thường: CeilingPrice gốc bằng 0 (được chuẩn hóa thành NULL).' ",
    "COMMENT ON COLUMN daily_stock_raw.anomaly_floor_zero IS 'Cờ dữ liệu SSI bất thường: FloorPrice gốc bằng 0 (được chuẩn hóa thành NULL).' ",
    "COMMENT ON COLUMN daily_stock_raw.anomaly_reason IS 'Danh sách nguyên nhân anomaly của các trường giá tham chiếu/trần/sàn, phân tách bằng dấu phẩy.'",
...
VIEW_STATEMENTS: list[str] = [
    ...
    SELECT
        ...
        r.floor_price,
        r.anomaly_ref_zero,
        r.anomaly_ceiling_zero,
        r.anomaly_floor_zero,
        r.anomaly_reason,
        r.open_price,
        r.high_price,
        r.low_price,
    ...
    "COMMENT ON COLUMN vw_daily_stock_llm.anomaly_ref_zero IS 'Cờ anomaly: RefPrice gốc từ SSI bằng 0, đã chuẩn hóa thành NULL.'",
    "COMMENT ON COLUMN vw_daily_stock_llm.anomaly_ceiling_zero IS 'Cờ anomaly: CeilingPrice gốc từ SSI bằng 0, đã chuẩn hóa thành NULL.'",
    "COMMENT ON COLUMN vw_daily_stock_llm.anomaly_floor_zero IS 'Cờ anomaly: FloorPrice gốc từ SSI bằng 0, đã chuẩn hóa thành NULL.'",
    "COMMENT ON COLUMN vw_daily_stock_llm.anomaly_reason IS 'Các nguyên nhân anomaly theo bản ghi daily raw, phân tách bằng dấu phẩy.'",
...
LEGACY_MIGRATIONS: list[str] = [
    ...
    INSERT INTO daily_stock_raw (
        ...
        anomaly_ref_zero, anomaly_ceiling_zero, anomaly_floor_zero, anomaly_reason,
    ...
    SELECT
        ...
        ceiling_price,
        floor_price,
        COALESCE(ref_price = 0, FALSE),
        COALESCE(ceiling_price = 0, FALSE),
        COALESCE(floor_price = 0, FALSE),
        CONCAT_WS(
            ',',
            CASE WHEN ref_price = 0 THEN 'ref_price_zero' END,
            CASE WHEN ceiling_price = 0 THEN 'ceiling_price_zero' END,
            CASE WHEN floor_price = 0 THEN 'floor_price_zero' END
        ),
        open_price,
        high_price,
        low_price,
    ...
    ON CONFLICT (ticker, trading_date) DO UPDATE SET
        ...
        anomaly_ref_zero = EXCLUDED.anomaly_ref_zero,
        anomaly_ceiling_zero = EXCLUDED.anomaly_ceiling_zero,
        anomaly_floor_zero = EXCLUDED.anomaly_floor_zero,
