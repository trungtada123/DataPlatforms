...
class FinancialReportsImportTests(TestCase):
    ...
    def test_canonical_financial_config_facade_is_legacy_bridge_free(self) -> None:
        from agents.financial_agent import config as financial_config_module
        from agents.financial_agent.config import (
            FinancialReportsToolSettings as CanonicalFinancialReportsToolSettings,
            get_financial_reports_settings as get_canonical_financial_reports_settings,
        )
        from config.financial import FinancialSettings
        from stock_etl.financial_reports_tool.config import (
            FinancialReportsToolSettings as LegacyFinancialReportsToolSettings,
            get_financial_reports_settings as get_legacy_financial_reports_settings,
        )

        with open(financial_config_module.__file__, "r", encoding="utf-8") as stream:
            module_source = stream.read()

        self.assertNotIn("stock_etl.financial_reports_tool.config", module_source)
        self.assertNotIn("from stock_etl", module_source)
        self.assertNotIn("import stock_etl", module_source)

        self.assertIs(CanonicalFinancialReportsToolSettings, FinancialSettings)
        self.assertEqual(LegacyFinancialReportsToolSettings.__name__, "FinancialReportsToolSettings")

        canonical_settings = get_canonical_financial_reports_settings()
        legacy_settings = get_legacy_financial_reports_settings()
        self.assertEqual(asdict(legacy_settings), asdict(canonical_settings))
