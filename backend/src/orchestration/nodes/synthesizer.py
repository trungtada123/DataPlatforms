...
class FinalSynthesizer:
    ...
    def _compact_merged_context_for_prompt(self, merged_context: MergedContext) -> dict[str, Any]:
        ...
        for item in payload.get("tool_summaries", []):
            ...
            if summary_item.get("tool_name") == "financial_reports":
                report_contexts = summary_item.get("report_contexts") or []
                if isinstance(report_contexts, list):
                    trimmed_contexts: list[dict[str, Any]] = []
                    for item in report_contexts[:4]:
                        if not isinstance(item, dict):
                            continue
                        trimmed_contexts.append(
                            {
                                ...
                                "page": item.get("page"),
                                "preview": self._truncate_text(str(item.get("preview") or ""), 220),
                            }
                        )
                    summary_item["report_contexts"] = trimmed_contexts
