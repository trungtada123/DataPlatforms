...
def run_financial_agent(state: OrchestrationState) -> dict[str, Any]:
    ...
    return _run_agent_node(
        ...
        query_resolver=_resolve_financial_query,
...
def _resolve_financial_query(state: OrchestrationState) -> str:
    """Ưu tiên tool_queries.financial_reports từ intent plan, giữ câu metric gốc khi cần."""
    ...
    if isinstance(tool_queries, dict):
        for key, value in tool_queries.items():
            normalized_key = str(key).strip().lower()
            if normalized_key in {"financial_reports", "financial"} or normalized_key.endswith(
                (".financial_reports", "_financial_reports", ".financial", "_financial")
            ):
                planned_query = " ".join(str(value or "").split())
                if planned_query:
                    return planned_query
