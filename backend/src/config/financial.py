...
class FinancialSettings:
    ...
    minio_bucket: str = "financial-reports-parsed"
    minio_prefix: str = "parsed"
    raw_bucket: str = "financial-reports-raw"
    parsed_bucket: str = "financial-reports-parsed"
    ingest_queue: str = "financial_ingest_jobs"
...
def get_financial_settings() -> FinancialSettings:
    ...
    settings = FinancialSettings(
        ...
        minio_bucket=(
            os.getenv("FINANCIAL_REPORTS_MINIO_BUCKET", "financial-reports-parsed").strip()
            or "financial-reports-parsed"
        ),
        minio_prefix=(os.getenv("FINANCIAL_REPORTS_MINIO_PREFIX", "parsed").strip().strip("/") or "parsed"),
        raw_bucket=(
            os.getenv("FINANCIAL_REPORTS_RAW_BUCKET", "financial-reports-raw").strip()
            or "financial-reports-raw"
        ),
        parsed_bucket=(
            os.getenv("FINANCIAL_REPORTS_PARSED_BUCKET", "financial-reports-parsed").strip()
            or "financial-reports-parsed"
        ),
        ingest_queue=(
            os.getenv("FINANCIAL_INGEST_QUEUE", "financial_ingest_jobs").strip() or "financial_ingest_jobs"
        ),
    )
