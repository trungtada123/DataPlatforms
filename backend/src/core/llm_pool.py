"""Shared LLM key pools and retry helpers."""
...
def get_gemini_pool(settings: GeminiSettingsLike, *, generation_config: dict[str, Any] | None = None, before_request: Callable[[str], None] | None = None, after_success: Callable[[str], None] | None = None) -> GeminiKeyPool:
    return GeminiKeyPool(settings, generation_config=generation_config, before_request=before_request, after_success=after_success)


def get_groq_pool(settings: GroqSettingsLike, *, before_request: Callable[[str], None] | None = None, after_success: Callable[[str], None] | None = None) -> GroqKeyPool:
    return GroqKeyPool(settings, before_request=before_request, after_success=after_success)


def call_with_retry(operation: Callable[[], _T], *, max_retries: int, retry_delay_seconds: float, sleep_func: Callable[[float], None] | None = None, on_retry: Callable[[int, Exception], None] | None = None) -> _T:
    if max_retries < 0:
    ...
    for attempt in range(max_retries + 1):
        try:
            result = operation()
            record_llm_call(provider=_resolve_provider_name(operation), status="success")
            return result
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            provider = _resolve_provider_name(operation)
            record_llm_call(provider=provider, status="retry" if attempt < max_retries else "error")
