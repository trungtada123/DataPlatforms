...
def rerank_candidate(query: str, candidate: ReportCandidate) -> None:
    ...
    if focus == "amount":
        ...
        elif chunk_type in {"table_full", "table"}:
            bonus = scoring.retrieval_table_full_context_bonus
            if chunk_type == "table" and metric_targets and any(
                metric_name in hay for metric_name in metric_targets
            ):
                bonus += 4.0
                why.append("html_table_metric_bonus=4.0")
            score += bonus
            why.append(f"table_context_bonus={bonus:.1f}")
        else:
