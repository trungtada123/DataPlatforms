...
from core.vector_store import FinancialReportsQdrantStore, QdrantCollectionMissingError
...
class FinancialReportsQueryService:
    ...
    def ask(
        ...
        *,
        ...
        try:
            ...
            limitations.append(
                "LLM rewrite (Groq) không khả dụng nên đã fallback về heuristic retrieval queries."
            )
            if trace_collector:
                trace_collector.add_event(
                    ...
                    status="warning",
                    detail=str(exc),
                    metadata={"mode": "heuristic_fallback", "provider": "groq"},
                )
        ...
        if not vectors:
            return self._no_data_response(
                ...
                plan=plan,
                filters=filters,
                limitations=limitations + ["Không tạo được embedding vector cho truy vấn BCTC."],
                debug=debug,
            )

        try:
            candidate_lists = self._query_candidates(vectors, query_filter)
        except QdrantCollectionMissingError:
            vector_size = len(vectors[0])
            created = self.store.ensure_collection(vector_size=vector_size)
            ingest_hint = (
                f"Collection Qdrant `{self.settings.qdrant_collection}` "
                + (
                    "vừa được tạo nhưng chưa có chunk nào."
                    if created
                    else "chưa tồn tại."
                )
                + " Cần chạy pipeline ingest BCTC (Airflow DAG `financial_ingest` hoặc workers download → parse → chunk → embedding)."
            )
            try:
                candidate_lists = self._query_candidates(vectors, query_filter)
            except QdrantCollectionMissingError:
                candidate_lists = [[] for _ in vectors]
            if not any(candidate_lists):
                return self._no_data_response(
                    question=question,
                    plan=plan,
                    filters=filters,
                    limitations=limitations + [ingest_hint],
                    debug=debug,
                )
            limitations.append(ingest_hint)
        ...
        if not ranked:
            empty_limitations = limitations + [
                "Qdrant không trả về retrieval hit phù hợp với filter/query hiện tại."
            ]
            return self._no_data_response(
                question=question,
                plan=plan,
    ...
    def _query_candidates(self, vectors: list[list[float]], query_filter: Any) -> list[list]:
        return [
    ...
    def _no_data_response(
        ...
        *,
        ...
        ingest_notes = [item for item in limitations if "ingest BCTC" in item or "Collection Qdrant" in item]
        if ingest_notes:
            summary = (
                "Chưa có dữ liệu báo cáo tài chính trên Qdrant cho câu hỏi này. "
                "Cần chạy pipeline ingest BCTC trước khi truy vấn lợi nhuận/số liệu quý."
            )
        else:
    ...
    def _looks_like_metric_match(payload: dict[str, Any], metric_targets: list[str]) -> bool:
        ...
        if chunk_type in {"table", "table_full"}:
            return any(table_payload_has_metric(payload, metric_name) for metric_name in metric_targets)
