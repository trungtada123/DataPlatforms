...
from .table_html import extract_row_values_from_table_payload
    ...
    def _extract_explicit_amount_answer(
        ...
        for metric_name in metric_targets:
            for item in context_items:
                ...
                row_values_raw = metadata.get("row_values")
                row_values = row_values_raw if isinstance(row_values_raw, dict) else {}
                if not row_values:
                    row_values = extract_row_values_from_table_payload(item, metric_name) or {}
                if not row_values:
                    continue
                if isinstance(row_values_raw, dict) and row_values_raw:
