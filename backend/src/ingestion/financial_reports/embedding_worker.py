...
def chunk_payload_to_chunk(payload: dict[str, Any]) -> Chunk:
    ...
    text = str(payload.get("content_for_embedding") or metadata.get("content_for_embedding") or "")
    if not text.strip():
        raise ValueError(f"Chunk payload {payload.get('chunk_id') or '<unknown>'} is missing content_for_embedding.")
