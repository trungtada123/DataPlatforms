...
def chunks_from_provider_payload(job: FinancialChunkJob, payload: dict[str, Any]) -> list[Chunk]:
    """Convert LandingAI/provider chunks already stored in parse JSON into embedding chunks."""
    ...
    for index, item in enumerate(raw_chunks):
        ...
        page = item.get("page")
        chunk_type = str(item.get("type") or item.get("chunk_type") or "text")
        heading_path = item.get("heading_path")
        section_title = None
        if isinstance(heading_path, list) and heading_path:
            section_title = " > ".join(str(part) for part in heading_path if str(part).strip())

        raw_meta = item.get("raw") if isinstance(item.get("raw"), dict) else {}
        chunk_id = str(
            item.get("chunk_id")
            or raw_meta.get("chunk_id")
            or f"{job.doc_id}_provider_{index:04d}"
        )
        retrieval_id = f"financial_report_vi_{chunk_id}"
        metadata = {
            "doc_id": job.doc_id,
            "ticker": job.ticker,
...
class FinancialReportChunkWorker:
    ...
    def _process_job(
        ...
        *,
        ...
        payload = self._build_parser_payload(job, markdown_text, json_text)
        chunks = chunks_from_provider_payload(job, payload)
        if not chunks:
            parsed = parse_landingai_output(payload)
