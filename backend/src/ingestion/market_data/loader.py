"""Load/persist helpers and public ingestion workflows for market data."""
...
from .extractor import (
    fetch_daily_rows_for_day,
    fetch_daily_rows_for_range,
...
def _upsert_daily_history_for_symbol(
    ...
    with session_factory.begin() as session:
        ...
        upsert_feature_rows(session, compute_daily_feature_rows(merged_rows, listed_shares))
...
def bootstrap_history(
    ...
    if end_date == local_today(settings) and is_trading_day(end_date):
        intraday_summary = refresh_intraday(symbols=active_symbols, trading_date=end_date)
