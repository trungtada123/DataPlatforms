...
class BackendWorkflowTests(TestCase):
    ...
    def test_market_only_query(self) -> None:
        ...
        with patch("orchestration.nodes.classifier.IntentClassifier") as classifier_cls, patch(
            "orchestration.nodes.tools.market_answer",
    ...
    def test_hybrid_query_uses_multiple_tools(self) -> None:
        ...
        plan = IntentPlan(
            ...
            tool_queries={"market": query, "news": query},
        ...
        with patch("orchestration.nodes.classifier.IntentClassifier") as classifier_cls, patch(
            "orchestration.nodes.tools.market_answer",
            return_value=_agent_result(ToolName.MARKET, query, "Gia HPG 28.1"),
        ), patch(
            "orchestration.nodes.tools.news_answer",
            return_value=_agent_result(ToolName.NEWS, query, "HPG co tin moi"),
        ), patch(
            "orchestration.nodes.synthesizer.FinalSynthesizer"
        ) as synth_cls:
            classifier_cls.return_value.classify.return_value = plan
    ...
    def test_workflow_error_is_controlled(self) -> None:
        with patch("orchestration.workflow.get_workflow") as workflow_getter:
