...
class FinancialReportsSynthesisTests(TestCase):
    ...
    def test_rewrite_prompt_template_supports_format_question_placeholder(self) -> None:
        from agents.financial_agent.synthesis import REWRITE_PROMPT_TEMPLATE

        rendered = REWRITE_PROMPT_TEMPLATE.format(question="Lợi nhuận sau thuế quý 2/2025 của ACB là bao nhiêu?")
        self.assertIn("Lợi nhuận sau thuế quý 2/2025 của ACB là bao nhiêu?", rendered)
        self.assertIn('"normalized_question"', rendered)
