...
class BackendLandingAITests(TestCase):
    ...
    def test_agentic_doc_parse_extracts_markdown_chunks_pages_and_tables(self) -> None:
        ...
        self.assertEqual(result.json_payload["page_span"], {"start_page_idx": 0, "end_page_idx": 1, "page_count": 2})
        self.assertEqual(len(result.json_payload["pages"]), 2)
        self.assertEqual(result.json_payload["pages"][0]["page_number"], 1)
        ...
        self.assertIn("bbox", result.json_payload["chunks"][0])
        self.assertIn("heading_path", result.json_payload["chunks"][0])
        self.assertEqual(result.json_payload["tables_count"], 1)
        self.assertEqual(result.json_payload["tables"][0]["table_id"], "table_0")
        self.assertIn("headings", result.json_payload)
        self.assertEqual(result.json_payload["provider_metadata"]["source"], "agentic-doc.parse.parse_documents")
