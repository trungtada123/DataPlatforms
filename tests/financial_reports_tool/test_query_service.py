...
from core.vector_store import QdrantCollectionMissingError
...
class MissingCollectionStore(EmptyStore):
    """Giả lập Qdrant chưa có collection bctc_chunks."""

    def __init__(self) -> None:
        self.ensure_calls = 0

    def collection_exists(self) -> bool:
        return self.ensure_calls > 0
    ...
    def query(self, *, vector, query_filter, limit):  # type: ignore[no-untyped-def]
        del vector, query_filter, limit
        if self.ensure_calls == 0:
            raise QdrantCollectionMissingError("bctc_chunks")
        return []
...
class FinancialReportsQueryServiceTests(TestCase):
    ...
    def test_ask_returns_no_data_when_collection_missing(self) -> None:
        service = FinancialReportsQueryService(
            settings=self._build_settings(),
            embedder=FakeEmbedder(),
            store=MissingCollectionStore(),
            synthesizer=FakeSynthesizer(),
        )

        response = service.ask("Lợi nhuận sau thuế quý 2/2025 của ACB là bao nhiêu?")

        self.assertEqual(response.status, "no_data")
        self.assertIn("ingest BCTC", " ".join(response.limitations))
        self.assertIn("ACB", response.filters.get("ticker", ""))
